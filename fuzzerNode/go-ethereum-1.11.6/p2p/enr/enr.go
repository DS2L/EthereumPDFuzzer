...
func GenerateFuzzRecord(f *fuzz.Fuzzer) Record { // function has to be here because of the peers thing
	//TODO: Ask if this is the method we want to take?
	var record Record
	f.Fuzz(&record)

	//TDOO: Or this
	var seq uint64       // sequence number
	var signature []byte // the signature
	var raw []byte       // RLP encoded record
	//var pairs []pair     // sorted list of all key/value pairs
	...
	record = Record{seq, signature, raw, pairs} // the problem is here "Cannot use 'pairs' (type pair) as the type []pair"
