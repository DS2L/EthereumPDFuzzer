...
// Return Local UDPv5
func (t *UDPv5) ReturnLocalUDPv5() *UDPv5 {
	return t
}
...
func (t *UDPv5) newLookup(ctx context.Context, target enode.ID) *lookup {
	return newLookup(ctx, t.tab, target, func(n *node) ([]*node, error) {
		//fmt.Println("Node passed in to newLookup" + n.ID().String() + n.IP().String())
...
func (t *UDPv5) FuzzPING(n *enode.Node, reqid []byte) (error) {
	resp := t.fuzzcall(n, v5wire.PongMsg, &v5wire.Ping{ENRSeq: t.localNode.Node().Seq()}, reqid)
        defer t.callDone(resp)
...
func (t *UDPv5) FuzzFindNode(n *enode.Node, distances []uint, reqid []byte) ([]*enode.Node, error) {
	resp := t.fuzzcall(n, v5wire.NodesMsg, &v5wire.Findnode{Distances: distances}, reqid)
	return t.waitForNodes(resp, distances)
}

func (t *UDPv5) FuzzTalkRequest(n *enode.Node, protocol string, request []byte, reqid []byte) ([]byte, error) {
	req := &v5wire.TalkRequest{Protocol: protocol, Message: request}
        resp := t.fuzzcall(n, v5wire.TalkResponseMsg, req, reqid)
        defer t.callDone(resp)
        select {
        case respMsg := <-resp.ch:
                return respMsg.(*v5wire.TalkResponse).Message, nil
...
func (t *UDPv5) findnode(n *enode.Node, distances []uint) ([]*enode.Node, error) {
	//fmt.Println("Node passed in to findnode, findNode" + n.ID().String() + n.IP().String())
...
func (t *UDPv5) call(node *enode.Node, responseType byte, packet v5wire.Packet) *callV5 {
	...
	select {
	case t.callCh <- c: // The error is here
...
// call sends the given call and sets up a handler for response packets (of message type
// responseType). Responses are dispatched to the call's response channel.
func (t *UDPv5) fuzzcall(node *enode.Node, responseType byte, packet v5wire.Packet, reqid []byte) *callV5 {
	c := &callV5{
		node:         node,
		packet:       packet,
		responseType: responseType,
...
func (t *UDPv5) sendCall(c *callV5) {
	...
	addr := &net.UDPAddr{IP: c.node.IP(), Port: c.node.UDP()} // take these
...
// Darren's Fuzzing Code, this code is a wrapper for the get node function below since UDPv5 functions use an actual node not an EID
func (t *UDPv5) CollectgetNode(id enode.ID) *enode.Node {
	return t.getNode(id)
}
...
func (t *UDPv5) handle(p v5wire.Packet, fromID enode.ID, fromAddr *net.UDPAddr) {
	...
	case *v5wire.Nodes:
		// Kai Li's Log 
		/*
		fmt.Printf("From Node: %s %d\n", fromAddr.IP, fromAddr.Port)
		i := 0
		fmt.Printf("p.Nodes Length %d", len(p.Nodes))
		for _, record := range p.Nodes {
			fmt.Printf("Node %d\n", i)
			fmt.Println("Record ")
			fmt.Print(record)
			fmt.Printf("Record Sequence: %d\n", record.Seq())
			fmt.Printf("Record Signature: %s\n", record.Signature())
			i++
		}
		*/
	...
	case *v5wire.TalkResponse:
		/*
		fmt.Println("Talk Response Received!")
		fmt.Printf("TR %d\n", p.ReqID)
		fmt.Printf("TR Message %d\n", p.Message)
		*/
		t.handleCallResponse(fromID, fromAddr, p)
	}
}
...
func generateNodesHandleFindNode(f *fuzz.Fuzzer) []*enode.Node {
	var nodes []*enode.Node
	var fuzzedID [32]byte // The generated ID must be a byte array
	for i := 1; i <= findnodeResultLimit; i++ {
		f.Fuzz(&fuzzedID)
		fuzzedRecord := enr.GenerateFuzzRecord(f)
		newNode, _ := enode.NewWithoutValidation(&fuzzedRecord)
		nodes = append(nodes, newNode)
		//fmt.Println(fmt.Sprintf("Generated Node %d: %s", i, newNode))
	}
	return nodes
}

// Darren's Fuzzing code to edit
func (t *UDPv5) GenerateNodesAndSendMessage(f *fuzz.Fuzzer, p *v5wire.Findnode, fromID enode.ID, fromAddr *net.UDPAddr) {
	nodes := generateNodesHandleFindNode(f)
...
func (t *UDPv5) handleFindnode(p *v5wire.Findnode, fromID enode.ID, fromAddr *net.UDPAddr) {
	//fmt.Println("FindNode Request Received:")
	for i := 0; i < len(p.Distances); i++ {
