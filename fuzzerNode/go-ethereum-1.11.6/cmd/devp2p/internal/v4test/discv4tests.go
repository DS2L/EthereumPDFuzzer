...
// FindnodeFuzz sends a FINDNODE request after performing the endpoint proof and fuzzes the target key
func FindnodeFuzz(t *utesting.T) {
	te := newTestEnv(Remote, Listen1, Listen2)
	defer te.close()
	bond(t, te)
...
// NeighborsFuzz sends a NEIGHBORS packet with fuzzed nodesS
func NeighborsFuzz(t *utesting.T) {
	te := newTestEnv(Remote, Listen1, Listen2)
	defer te.close()
	bond(t, te)
	...
	for {
		// Send neighbors packet
		neighbors := v4wire.Neighbors{Expiration: uint64(time.Now().Add(expiration).Unix())}
		...
		for i := 1; i <= 12; i++ {
			...
			for {
				...
				if ip.To4() != nil {      // If the ip address is a valid IP address
					finalIP = ip.To4()
			...
			*/
			...
			neighbors.Nodes = append(neighbors.Nodes, nodePayload) // Add the 12 fuzzing nodes to the neighbor message.
		}
...
var AllTests = []utesting.Test{
	//{Name: "Fuzz/Findnode", Fn: FindnodeFuzz}, Enable this test to run the fuzzing test
	//{Name: "Fuzz/NeighborsMessage", Fn: NeighborsFuzz}, Enable this test to run the fuzzing test
